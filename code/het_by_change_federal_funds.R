...
district_finances_dat = merge(district_finances_dat, district_finances_lagged, by=c("year", "leaid"), suffixes = c("", "_lag"))

district_finances_dat[, rev_change := rev_total - rev_total_lag]
district_finances_dat[, rev_fed_change := rev_fed_total - rev_fed_total_lag]
...
district_finances <- merge(district_finances, district_change_rev, by = c('leaid','fips', 'year'),  all.x = TRUE)
table(is.na(district_finances$change_qtile)) ## losing A LOT of observations
...
left = left[!is.na(instance_leaid)] # Dropping rows that have zero past instances
left = left[instance_leaid != leaid] # Removing instances where the district itself wins here 
N_past_refs_dat = left[,.(big_rev_change_districts = uniqueN(instance_leaid)), by = .(leaid, year)]
...
analysis_dat[is.na(big_rev_change_districts), big_rev_change_districts:=0]
